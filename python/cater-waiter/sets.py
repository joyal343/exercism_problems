...
def check_drinks(drink_name, drink_ingredients):
    ...
    if set_ingredients.isdisjoint(ALCOHOLS):
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    if set_ingredients.issubset(VEGETARIAN):
        return dish_name+': VEGETARIAN'
    if set_ingredients.issubset(PALEO):
        return dish_name+': PALEO'
    if set_ingredients.issubset(KETO):
        return dish_name+': KETO'
    if set_ingredients.issubset(OMNIVORE):
...
def singleton_ingredients(dishes, intersection):
    ...
    for item in dishes:
        special_ingredients = special_ingredients.union(item.difference(intersection))
